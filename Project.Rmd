...
reviewtext_lemma <- tokens_lemma %>% 
  group_by(id) %>% 
  summarize(text.review = str_c(word, collapse = " ")) %>% 
  ungroup() %>% 
  select(text.review) %>% 
  as_vector()
```
By doing this the number of unique words are reduced from around 6000 to around 4600 words, which should prove quite beneficial. 

Bigrams are now created. Then the most common bigrams are displayed. 

```{r}
bigrams <- data_clean %>%
  mutate(text.review = reviewtext_lemma) %>% # add the lemma cleaning
...
We want to recombine words by using 'tidyr' function 'unite'. This will be used to analyzing the bigrams.
The purpose is to threat the bigram as a 'term in a document'. Therefor we can look at the tf_idf values
of bigrams and visualize them.
...
#### COMMENT ON THE TF_IDF #####

The interesting thing is now to visualize the relationship between all words. To this we will use the
package igraph. Before doing this we will need to create the graph from a data frame. Where we take a 
data frame of edges with columns for 'from', 'to'. Which is equal to n.
