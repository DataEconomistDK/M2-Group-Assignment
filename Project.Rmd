...
In this section we will analyse whether we can predict if the review is good, her categorized as rating 4 or 5. We will apply supervised machine learning to do so and create a binary logistic model. 
...
log_model = cv.glmnet(sparse_words, j, family = "binomial", keep = TRUE, intercept = FALSE
)
```
...
```{r}
a = table(classifications$reviews.rating == 1, classifications$probability > 0.7)
a
```
