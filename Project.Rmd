...
We wil start with the Bing lexicon. The Bing lexicon categorizes words in a binary fashion as positive or negative with no weighting. Here, we are using the function get_sentiment to get a specific sentiment lexicon and inner_join to join the lexicon with tokenized data. After this we can count the sentiments.
...
What's interesting here is that all reviews, except one star rating, have a positive score. Of course this could also be, because it categorizes negative words as positive words or vice versa, like "it's amazing how bad it is" may categorize "amazing" (4) as a positive word with a high value even if it used negatively. This could be fixed with further analysis to remove the effect of negatives, but is not within the current time scope of this project. 
...
```{r}
tweets_split = data_raw %>% select(id) %>% initial_split()
train_data = training(tweets_split)
test_data = testing(tweets_split)
```

```{r}
sparse_words = tokens_nlp %>% count(id, word) %>% inner_join(train_data) %>% cast_sparse(id, word, n)
```

```{r}
word_rownames = as.integer(rownames(sparse_words))
joined = data_frame(id = word_rownames) %>% left_join(data_raw %>% select(id, reviews.rating))
joined
```
...
joined$reviews.rating1[joined$reviews.rating == 1] <- 1
joined$reviews.rating1[joined$reviews.rating == 2] <- 2
joined$reviews.rating1[joined$reviews.rating == 3] <- 3
joined$reviews.rating1[joined$reviews.rating == 4] <- 4
joined$reviews.rating1[joined$reviews.rating == 5] <- 5

joined$reviews.rating1

j = joined$reviews.rating1 == 1
log_model = cv.glmnet(sparse_words, j, family = "binomial", parallel = TRUE, keep = TRUE
)
```
...
classifications = tokens_nlp %>%
  ...
  mutate(probability = plogis(intercept + score)) %>%
  left_join(data_raw %>%
    select(reviews.rating, id), by = "id")
```

```{r}
a = table(classifications$reviews.rating == 1, classifications$probability > 0.5)

a
```

```{r}
yardstick::accuracy(a)
```
