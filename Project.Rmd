...
##LSA
Latent Semantic Analysis or simply LSA is a techique to identify and analyze the cooccurrences of words across documents. Coorccurrence suggest that the words are somewhat correlated, either by being synonymous or reflect a shared concept. Examples of shared concepts could be colors or cities.
...
Then we fit the LSA scaling model to the dfm, where we set nd, the number of dimensions, to five. 
...
```{r}
data_lsa_loading <- data_dfm$features %>%
  as.data.frame() %>%
...
Here, there's three different clusters and 25 outliers. One of the clusters is quiet big and have 3042 out of the 3542 features. The others are smaller and there's 25 outliers, which doesn't have a cluster. There could be lot more clusters, because each cluster should have a minimum of 100 features, but the function only makes three clusters. 
...
```{r}
############## Denne del fungere ikke endnu Mathias.
